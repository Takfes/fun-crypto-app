...
class Traid(bt.Strategy):
    ...
    def __init__(self):
        ...
        # indicators
        self.fast_ema = bt.indicators.EMA(self.data.close, period=self.params.fastema, plotname=f'{self.p.fastema}-EMA')
        self.slow_ema = bt.indicators.EMA(self.data.close, period=self.params.slowema, plotname=f'{self.p.slowema}-EMA')
    ...
    def notify_trade(self, trade):
        if not trade.isclosed:
            return
        self.log(
            '(7) OPERATION PROFIT, GROSS %.2f, NET %.2f' % (trade.pnl, trade.pnlcomm)) if trade.pnl > 0 else self.log(
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # OPEN LONG
            if (self.dc[0] > self.lines.bbt) and (self.dc[0] > self.wma):
                self.log(f"SIGNAL NUMBER #{self.signal_number}")
                self.signal_number += 1
                # if (self.dataclose[0] > self.lines.bbt) and (self.dataclose[0] > self.wma):
                self.sizer()
                self.log(
            ...
            if self.params.short_positions:
                ...
                if (self.dc[0] < self.lines.bbb) and (self.dc[0] < self.wma):
                    self.log(f"SIGNAL NUMBER #{self.signal_number}")
                    self.signal_number += 1
                    # if (self.dataclose[0] < self.lines.bbb) and (self.dataclose[0] < self.wma):
                    self.sizer()
                    self.log(
        ...
        # CLOSE POSITIONS
            ...
            if self.position.size > 0:
                ...
                if self.datahigh[0] >= self.executed_price * (1 + self.params.takeprofit):
                    # self.close()
                    self.sell(exectype=bt.Order.Limit, size=self.size,
                ...
                elif (self.params.emergency_exit == 1) and (self.rsi < self.params.rsi_value_long) and (
                        self.datahigh[0] > self.executed_price):
                    self.close()
            ...
            if self.params.short_positions:
                if self.position.size < 0:
                    ...
                    if self.datalow[0] <= self.executed_price * (1 - self.params.takeprofit):
                        # self.close()
                        self.buy(exectype=bt.Order.Limit, size=self.size,
                                 price=self.executed_price * (1 - self.params.takeprofit))
                        self.log(
                            f'(3) CLOSE SHORT position at {self.executed_price * (1 - self.params.takeprofit):.2f}')
                        self.profit_loss = "profit"
                    elif (self.params.emergency_exit == 1) and (self.rsi > self.params.rsi_value_short) and (
                            self.datahigh[0] < self.executed_price):
                        self.close()
